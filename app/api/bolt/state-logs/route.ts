...
export async function POST(request: NextRequest) {
  try {
    ...
    const { company_id, start_date, end_date, limit = 100, offset = 0 } = body;
    ...
    const result = await boltService.getStateLogs(
      company_id,
      startDateObj,
      endDateObj,
      limit,
    ...
    console.error('Bolt state logs endpoint error:', error);
    return NextResponse.json(
